...
export default {
  ...
  methods: {
      async sendPing() {
            ...
            try {
              const url = this.port.value
                ? `http://localhost:3000/ping?ip=${this.ip.value}&port=${this.port.value}`
                : `http://localhost:3000/ping?ip=${this.ip.value}`;
              const response = await fetch(url);
              const data = await response.json();
              this.pingResult.value = data.message + (data.portStatus || '');
            } catch (error) {
              this.pingResult.value = 'Error: Unable to ping the IP.';
              console.error(error);
            } finally {
